...
public interface UserMapper {
    ...
    @Update("DELETE FROM m_user SET " + "username=#{username},avatar=#{avatar},email=#{email},password=#{password}," + "status=#{status},created=#{created},last_login=#{last_login},token=#{token} " + "WHERE username=#{old_username};")
